...
- Content:
  - Displays 4 test images in a 4x2 grid (row-wise for each image):
    - Column 1: Singly-Noisy (Y, BUSI images, treated as pseudo-clean).
    - Column 2: Denoised (f(Y) ≈ X, model output from Y).
- Appearance:
  - Singly-Noisy: Moderately grainy, anatomical details visible but noisy.
  - Denoised: Smoother, reduced speckle, clearer structures (e.g., lesion boundaries).
- Interpretation:
  - Good Result:
    - Denoised: Significant noise reduction compared to Singly-Noisy, sharp lesion boundaries, preserved textures. PSNR ~34–35 dB.
    - Denoised image is clearer than Singly-Noisy, approaching a hypothetical clean X.
  - Poor Result:
...
- Pretrain:
  ...
  - Poor: PSNR <30 dB (model didn’t learn to fill patches).
- Finetune:
...
- Finetune (test_results_finetune_noise{noise_std}.png):
  - Good:
    - Denoised: Reduced speckle compared to Singly-Noisy, clear anatomical structures.
    - Denoised closer to a hypothetical clean X than Singly-Noisy.
  - Poor:
...
def test_model(model, test_loader, config, checkpoint_path, save_dir="test_outputs_finetune"):
    ...
    with torch.no_grad():
        for i, (doubly_noisy, singly_noisy) in enumerate(tqdm(test_loader, desc="Testing Finetuned")):
            ...
            if i == 0:
                single_images = singly_noisy[:4].cpu()
                denoised_images = output[:4].cpu()

    avg_test_loss = test_loss / num_batches
    avg_test_psnr = test_psnr / num_batches

    print(f"📊 Finetuned Test Results (Singly-Noisy Input, Noise Std={config.noise_std}):")
    print(f"Average Test Loss: {avg_test_loss:.4f}")
    print(f"Average Test PSNR: {avg_test_psnr:.2f} dB")

    visualize_results(single_images, denoised_images, save_dir, config.noise_std)

def visualize_results(single_images, denoised_images, save_dir, noise_std):
    # Denormalize images
    single_images = single_images * 0.5 + 0.5
    denoised_images = denoised_images * 0.5 + 0.5

    num_images = single_images.size(0)  # 4 images
    ...
    for i in range(num_images):
        ...
        plt.imshow(single_images[i].squeeze(), cmap='gray')

...
- Content:
  - Displays 4 test images in a 4x2 grid (row-wise for each image):
    ...
    - Column 2: Denoised (f(Y) ≈ X, model output from Y).
- Appearance:
...
- Interpretation:
  - Good Result:
    - Denoised: Significant noise reduction compared to Input, sharp lesion boundaries, preserved textures. PSNR ~34–35 dB, SSIM >0.90.
    - Denoised image is clearer than Input, approaching a hypothetical clean X.
  - Poor Result:
    - Denoised remains noisy (PSNR <32 dB, SSIM <0.85).
    - Blurring or loss of anatomical details (e.g., lesions indistinct).
    - Artifacts like streaks or unnatural textures.
  - Compare to PSNR/SSIM: PSNR >34 dB and SSIM >0.90 indicate effective denoising, but visual quality (sharpness, detail) is critical.
...
- Content:
  - Three subplots:
    ...
    - Middle: PSNR per Epoch (Train and Validation).
    - Right: SSIM per Epoch (Train and Validation).
- Appearance:
  - Loss Plot: Decreasing curves (e.g., from 0.019 to 0.007).
  - PSNR Plot: Increasing curves (e.g., from 31 dB to 34.5 dB).
  - SSIM Plot: Increasing curves (e.g., from 0.80 to 0.92).
- Interpretation:
  - Good Result:
    - Loss decreases steadily, converges to <0.01.
    - PSNR increases, stabilizes at >34 dB.
    - SSIM increases, stabilizes at >0.90.
    - Train and val curves are close (no overfitting).
  - Poor Result:
    - Loss plateaus early or diverges.
    - PSNR remains low (<32 dB), SSIM <0.85.
...
-------------------
- Pretrain:
  - Loss ~0.0100, PSNR ~33.00 dB, SSIM ~0.88: Indicates good Noise2Void reconstruction.
  - Good: PSNR >32 dB, SSIM >0.85, loss <0.01.
  - Poor: PSNR <30 dB, SSIM <0.80 (model didn’t learn to fill patches).
- Finetune:
  - Denoised (f(Y) vs. Y): Loss ~0.0075, PSNR ~34.80 dB, SSIM ~0.92. Shows effective denoising of singly-noisy images.
  - Good: PSNR >34 dB, SSIM >0.90, loss <0.01.
  - Poor: PSNR <32 dB, SSIM <0.85 (insufficient denoising).
- Comparison:
  - Fine-tuning PSNR/SSIM should be higher than pretraining, reflecting improved denoising.
  - Since X is unavailable, metrics are computed against Y, so focus on visual improvement.
...
def test_model(model, test_loader, config, checkpoint_path, save_dir="test_outputs_finetune"):
    ...
    with torch.no_grad():
        for i, (doubly_noisy, singly_noisy) in enumerate(tqdm(test_loader, desc="Testing Finetuned")):
            ...
            # Test with singly-noisy input: f(Y) ≈ X
